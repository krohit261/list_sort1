...
function checkJSON(data,res){
  if(!data[0]['Year of DSC']){
    res.send('`Year of DSC');
  }
  if(!data[0]['DSC Rank']){
    res.send('`DSC Rank` doesnt exist');
  }
  if(!data[0]['DSC Rank MARKS']){
    res.send('`DSC Rank MARKS` doesnt exist');
  }
}
...
function dscRank(newData,oldData,newRank,oldRank,dataInserted,index){
  if(parseInt(newRank) < parseInt(oldRank)){
    ...
    if(!dscMarks(newData,oldData,oldData['DSC Rank MARKS'],newData[index]['DSC Rank MARKS'],dataInserted,index)){
...
function dscMarks(newData,oldData,newMarks,oldMarks,dataInserted,index){
  if(parseFloat(newMarks) > parseFloat(oldMarks)){
    newData.splice(index,0,oldData);
    dataInserted = true;
    return true;
  }
  else if(parseFloat(newMarks) == parseFloat(oldMarks)){
...
function sortData(data, newData){
  for(let i=0;i<data.length;i++){
    ...
    for(let j=0;j<newData.length;j++){
      if(!dscYear(newData,data[i],data[i]['Year of DSC'],newData[j]['Year of DSC'],dataInserted,j)){
...
app.post('/upload', upload.single('excel'), function (req, res, next) {
  ...
  xlsx.utils.book_append_sheet(newWb,newWs,'FINAL ar');
